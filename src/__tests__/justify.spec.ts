...
describe('POST /api/justify', () => {
  it('returns text for a text who have less characters than justify size', async () => {
    ...
    const response = await request(app).post('/api/justify').send(text).set('Content-Type', 'text/plain');
  ...
  it('returns error message when the body is missing', async () => {
    const response = await request(app).post('/api/justify').send().set('Content-Type', 'text/plain');
